...
#include "synaptics/rmi_function.h"
...
#define CURRENT_TIME_MILLISECONDS ((jiffies * 1000) / HZ)
...
#define F11_QUERY_REGISTER 61
#define F11_DATA_REGISTER 6
#define NUMBER_OF_FINGERS 10
#define BYTES_PER_FINGER 8

//#define ERROR_REPORTING	

//sets up the refresh rate
char RefreshRate = 15;
...
//sets up the clock
unsigned long Start, Difference;

//sets up the absolute position report data points
char AbsolutePositionStringReport[NUMBER_OF_FINGERS * 2 * 8];

//six possible data points, and a comma
char StringBuffer[(RX * TX * 7)];
char DataBuffer[DATA_POINTS];
short ProcessedDataBuffer[DATA_POINTS / 2];
 ...
 */
static u32 touch_debug_mask = 2047;
...
static void test_reporting_populate_processed_data_buffer(void)
{
	int i;
	
	for(i = 0; i < sizeof(ProcessedDataBuffer) / 2; i++)
	{
		ProcessedDataBuffer[i] = (short) 0;	
	}
}

static void test_reporting_remove_noise(void)
{
	int i;
	short * TempPointer;
	
	//start at the end of the data and work towards the beginning
	for(i = sizeof(DataBuffer) / 2; i >= 0; i--)
	{
		//convert to a short pointer
		TempPointer = (short*)DataBuffer + i;
		
		//if the value is not noise, set it. Otherwise, ignore it completely
		if(*TempPointer < 500)
		{
			//fits perfectly in a char. Mult by 1000 to act like a floating point #.
			//normalizes to [0, 255]
			ProcessedDataBuffer[i] = (short) (*TempPointer * 1000 / 500 * 255 / 1000);
		}	
		//if the value is greater than 500, stop saving them because we have hit the noise region
		else
		{
			#ifdef ERROR_REPORTING
			if(i < 10)
...
{
	//store the front location of the data buffer
	//short *Data = (short*)DataBuffer;
	short *Data = ProcessedDataBuffer;	
	...
	//remove the first three RX rows
	Data += RX * 3;

	// - (RX * 3) makes three less rows of RX happen here too
	for (i = 0; i < (RX * TX) - (RX * 3); i++) 
	{
		TempShort = *Data;
...
static void get_electrode_dimensions(void)
{
	int NumberRX = 0;
	int NumberTX = 0;
	char *rx_buffer;
	...
	touch_i2c_write_byte(test_reporting_client, PAGE_SELECT_REGISTER, 3);

	touch_i2c_read(test_reporting_client, F55_BASE_CONTROL + 1, TX + RX, buffer);
	...
	for (i = 0; i < RX; i++) 
	{
		if (rx_buffer[i] != 0xFF)
		{
			TOUCH_INFO_MSG("RX %i: %i", NumberRX, rx_buffer[i]);
			NumberRX++;
		}
	}
	for (i = 0; i < TX; i++) 
	{
		if (tx_buffer[i] != 0xFF)
		{
			TOUCH_INFO_MSG("TX %i: %i", NumberTX, tx_buffer[i]);
			NumberTX++;
		}
	}

	touch_i2c_write_byte(test_reporting_client, PAGE_SELECT_REGISTER, 0);
}

static void read_mode_byte(void)
{
	char buffer[1];
	...
	touch_i2c_read(test_reporting_client, F54_QUERY_REGISTER + 2,
			1,
			buffer);

	TOUCH_INFO_MSG("This is the mode byte: %i", *buffer);
}

static void function_register_locations(int Offset, int Page)
{
	//setup the data
	char buffer[6];
	...
	int FunctionRegister = 0xEF - Offset - 6;

	//switch to page 0
	touch_i2c_write_byte(test_reporting_client, PAGE_SELECT_REGISTER, Page);

	//put the locations in buffer
	touch_i2c_read(test_reporting_client, FunctionRegister,
			6,
			buffer);
...
static void print_all_functions(void)
{
	//setup the data
	char buffer[1];
...
restart:
	//select the page
	touch_i2c_write_byte(test_reporting_client, PageSelectRegister, Page);

	//write the page down
	TOUCH_INFO_MSG("***page %i***", Page);

	//read if the page select is in the standard location
	touch_i2c_read(test_reporting_client, 0xEF,
			1,
			standardlocation);
...
top:
	...
	touch_i2c_read(test_reporting_client, StartRegister + Offset,
			1,
			buffer);
...
static void test_reporting_get_finger_positions(void)
{
	char FingerData[NUMBER_OF_FINGERS * BYTES_PER_FINGER];
	int i;
	short FingerCoordinates[NUMBER_OF_FINGERS * 2];
	int Position = 0;
	...
	//read the data
	touch_i2c_read(test_reporting_client, F11_DATA_REGISTER,
			BYTES_PER_FINGER * NUMBER_OF_FINGERS,
			FingerData);
	...
	{
		//if the finger is actually touching
		if(FingerData[i * BYTES_PER_FINGER] == 1 || FingerData[i * BYTES_PER_FINGER] == 2)
		{
			//extracts the lsb and msb and puts them in FingerCoorodinates
			FingerCoordinates[Position] = (short) (FingerData[i * BYTES_PER_FINGER + 2] << 8) | FingerData[i * BYTES_PER_FINGER + 1];
			FingerCoordinates[Position + 1] = (short) (FingerData[i * BYTES_PER_FINGER + 4] << 8) | FingerData[i * BYTES_PER_FINGER + 3];
	...
	AbsolutePositionStringReport[0] = '[';
	
	for(i = 0; i < Position; i++)
	{
		sprintf(TempString, "%i,", FingerCoordinates[i]);

		//add the string to the StringReport array
		strncpy(AbsolutePositionStringReport + StringPosition, TempString, strlen(TempString)); 

		StringPosition += strlen(TempString);	
	}

	//ends the string and gets rid of the trailing ','
	AbsolutePositionStringReport[StringPosition] = ']';
	AbsolutePositionStringReport[StringPosition + 1] = '\0';	
}
...
static void get_image_data(struct work_struct *work)
{
	...
	//check the clock
	#ifdef ERROR_REPORTING	
	Difference = CURRENT_TIME_MILLISECONDS - Start;

	if(Difference > 30000 && RefreshRate > 10)
	{
		TOUCH_INFO_MSG("%i okay %i good %i great %i", RefreshRate, OkayReadings, GoodReadings, GreatReadings);
	...
	//remove the noise from the report
	test_reporting_remove_noise();

	//print out an image report (from the DataBuffer variable into the StringBuffer variable)
	print_image_report();
	...
	//run this function again in 100 miliseconds
	queue_delayed_work(test_reporting_workqueue, &Task, msecs_to_jiffies(RefreshRate));
}

static void test_reporting_initialization(void)
{
	//this is debugging information
	get_electrode_dimensions();
	read_mode_byte();
	print_all_functions();

	//fill the processed data buffer with zeroes
	test_reporting_populate_processed_data_buffer();
	
	//get the register locations for everything on page 1
	function_register_locations(12, 0);
...
static void test_reporting_cleanup(void)
{
	//clean up all of the work queue stuff
	...
	if (likely(ts->ts_data.interrupt_status_reg & INTERRUPT_MASK_ABS0)) {
		...
		for (id = 0; id < ts->pdata->max_id; id++) {
			...
			case FINGER_STATE_PRESENT_VALID:
				...
				//add this
				TOUCH_INFO_MSG("debug mask: [%x]", touch_debug_mask);
...
/* show_fw_info
 *
 * show only the firmware informationproc_create("crash_jiffies", 0, NULL, &jiffies_proc_fops);
 */
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//here we set the client up
	test_reporting_client = client;

	//we initialize the test_reporting
	test_reporting_initialization();
...
static int test_reporting_proc_show(struct seq_file *m, void *v)
{
...
static int absolute_position_reporting_proc_show(struct seq_file *m, void *v)
{
	char Temp[] = "[8498,3084,]";
	
	test_reporting_get_finger_positions();

	//while the data is inaccurate (for whatever reason), keep getting the data
	while(strcmp(Temp, AbsolutePositionStringReport) == 0)
	{
		TOUCH_INFO_MSG("Inaccurate Report. Retrying.");
		test_reporting_get_finger_positions();
	}
	
	seq_printf(m, "%s", AbsolutePositionStringReport);
	
	return 0;
}

static int test_reporting_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, test_reporting_proc_show, NULL);
}

static int absolute_position_reporting_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, absolute_position_reporting_proc_show, NULL);
}

static const struct file_operations test_reporting_proc_fops = {
    .owner      = THIS_MODULE,
    .open       = test_reporting_proc_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = single_release,
};

static const struct file_operations absolute_position_reporting_proc_fops = {
    .owner      = THIS_MODULE,
    .open       = absolute_position_reporting_proc_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
    .release    = single_release,
};
...
{
	//there are how the userspace interfaces with the kernel
	proc_create("test_reporting", 0, NULL, &test_reporting_proc_fops);
	proc_create("absolute_position_reporting", 0, NULL, &absolute_position_reporting_proc_fops);
...
{
	//remove the proc entry on unitialization
	remove_proc_entry("test_reporting", NULL);
	remove_proc_entry("absolute_position_reporting", NULL);

	//clean up all of the test reporting stuff
	test_reporting_cleanup();
