...
WindowWidth = 400
WindowHeight = 800
PhoneScreenWidth = 1080.0
PhoneScreenHeight = 1920.0
...
def SetupGraphics(SampleDataObject):
	...
	for x in range(0, XSize):
		...
		for y in range(0, YSize):
			Shape = Rectangle(Point(y * YSpacing, x * XSpacing), Point((y + 1) * YSpacing, (x  +1) * XSpacing))
	...
	for x in range(0, XSize):
		for y in range(0, YSize):
			if Data[x][y] != PreviousData[x][y] or DrawOnChange == False:
				...
				if Data[x][y] >= 0:
					GraphicsArray[x][y].setFill("#" + Hex + Hex + Hex)
				else:
					GraphicsArray[x][y].setFill("#00" + Hex + Hex)
...
def Update(MatrixData, TouchPointData, OptimizedTouchPointData, OutlineData, DrawOnChange):
